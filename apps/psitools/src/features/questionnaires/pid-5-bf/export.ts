...
export function generateResponsesHTML(records: JsonObject): string {
  ...
  let html =
    ...
    `<tr class="table_header">
                      ...
                      <th style="width:230;"><p>Risposta</p></th>
                    </tr>
                  </thead>
                  <tbody>`;
...
export function generateScoresHTML(records: JsonObject): string {
  ...
  DOMAIN_MAP.domini.forEach((domain) => {
    let scoresSum = 0;
  ...
  const totalScore = domainsResults.reduce(
    (acc, domain) => acc + domain.scoresSum,
    0,
  );
  const totalQuestionsCount = domainsResults.reduce(
    (acc, domain) => acc + domain.questionsCount,
    0,
  );
  ...
  html +=
    `<tr class="table_subheader">
              <th style="width:345;"><p>Totale</p></th>
              <th style="width:200;"><p>` +
    totalAverageRounded +
    `</p></th>
              <th style="width:200;"><p>` +
    totalScore +
