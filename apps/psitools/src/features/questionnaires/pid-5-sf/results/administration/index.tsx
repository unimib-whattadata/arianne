...
const AdministrationResultsPid5SF: NextPage = () => {
  ...
  const renderScoreCards = () => {
    ...
    const mediaTotale =
      keysToShow.reduce((sum, { scoreKey }) => {
        const facetScore = score[scoreKey] ?? 0;
        const media = facetScore / 4;
        return sum + media;
      }, 0) / keysToShow.length;
  ...
  const renderDotGraph = () => {
    ...
    const graphHeight = selectedFacet === 'tutti' ? 4000 : 400;
...
export default AdministrationResultsPid5SF;
