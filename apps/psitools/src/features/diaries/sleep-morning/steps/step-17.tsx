...
export default function Step17() {
  ...
  const [qualitySleep, setQualitySleep] = useState(5);
  ...
  const handleIntensityChange = (value: number) => {
    setQualitySleep(value);
    setValue('qualitySleep', value);
  };
          ...
          <FormItem>
            <div className="grid grid-cols-2 gap-x-10 rounded-sm bg-white px-4 py-6">
              <FormLabel className="col-span-1 text-base font-normal">
                Valuta la qualità del tuo sonno durante questa notte
              </FormLabel>
              ...
              <div className="relative col-start-2 mt-2" ref={sliderRef}>
                <div
                  ...
                  style={{ left: getThumbPosition(qualitySleep) }}
                  onMouseDown={(_e) => {
                ...
                <FormControl>
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, #2D7C35 0%, #2D7C35 ${((qualitySleep - 1) / 9) * 100}%, #E0E0E0 ${((qualitySleep - 1) / 9) * 100}%, #E0E0E0 100%)`,
                    }}
                  />
                </FormControl>
              </div>
