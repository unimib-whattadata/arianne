...
export default function SleepMorning() {
  const api = useTRPC();
  ...
  const createDiary = useMutation(
    api.diaries.create.mutationOptions({
      onSuccess: (data) => {
        ...
        document.cookie = `sleep_morning=1; path=/; expires=${expires.toUTCString()}`;
  ...
  return (
    <>
      <h1 className="p-4 text-xl font-bold">Diario Sonno-Mattina</h1>
