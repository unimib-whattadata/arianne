...
export default function Food() {
  ...
  const dailyDiaries = React.useMemo(() => {
    ...
    return diaries.filter((d) => d.date.toDateString() === date.toDateString());
  }, [diaries, date]);

  const diaryDates = diaries?.map((d) => new Date(d.date));
  ...
  React.useEffect(() => {
    ...
    if (diaryForSelectedDay) setSelectedDiaryId(diaryForSelectedDay.id);
    else setSelectedDiaryId(undefined);
  }, [date, dailyDiaries, selectedDiaryId]);

  const handleDiaryClick = (diaryId: string) => setSelectedDiaryId(diaryId);
  ...
  const createDiary = useMutation(
    api.diaries.create.mutationOptions({
      onSuccess: (data) => {
        ...
        document.cookie = `food=1; path=/; expires=${expires.toUTCString()}`;
  ...
  return (
    <>
      <h1 className="p-4 text-xl font-bold">Diario Alimentare</h1>
      ...
      <div className="p-4">
        <div className="flex max-w-3xl flex-col rounded-[4px] bg-white p-4">
          <div className="flex gap-4">
            ...
            {/* Preview compilazioni con scrollbar verticale */}
            <div className="flex w-full flex-col overflow-hidden">
              <p className="text-darkest-blue mb-3 text-[20px] font-bold">
                Preview compilazioni
              </p>

              <div className="scrollbar-blue max-h-[calc(4*64px)] overflow-y-auto">
