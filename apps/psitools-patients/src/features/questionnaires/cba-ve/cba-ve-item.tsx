    ...
    <FormField
      ...
      render={({ field }) => {
        return (
          <div
            className={cn(
              'border-white mb-2 flex flex-col rounded-md border bg-card p-4 transition-colors duration-300',
                ...
                >
                  {Array.from(Array(5).keys()).map((i) => {
                    return (
                      <FormItem
                        key={i.toString()}
                        className="text-primary-400 flex w-20 items-center justify-center space-x-3"
                      >
                        <FormControl>
                          <RadioGroupItem
                            value={i.toString()}
